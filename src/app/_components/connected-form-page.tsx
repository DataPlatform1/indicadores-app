...
export default function ConnectedFormPage() {
  ...
  useEffect(() => {
    async function loadTemplate() {
      try {
        ...
        console.error(error);
        setFeedback(
          "No fue posible cargar los indicadores desde la base de datos.",
        );
      } finally {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setConfirmationId(payload.recordNumber ?? "");
      setFeedback("El formulario se guardó correctamente en PostgreSQL.");
      setRecentResults((current) => {
        const created: ResultSummary = {
          id: payload.recordNumber ?? crypto.randomUUID(),
          recordNumber: payload.recordNumber ?? "",
          process: selectedIndicator?.process ?? "",
          indicator: selectedIndicator?.name ?? "",
          submittedBy: sessionUser?.name ?? null,
          compliance: formValues.compliance,
          resultValue: Number(formValues.result),
          indicatorPercent: Number(formValues.indicatorPercentage),
          createdAt: new Date().toISOString(),
        };

        return [created, ...current].slice(0, 8);
      });
      router.push(
        `/formulario/exito?radicado=${encodeURIComponent(
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback(`Sesión iniciada como ${payload.user.name}.`);
      const resultsResponse = await fetch("/api/results");
      if (resultsResponse.ok) {
        const resultsPayload = (await resultsResponse.json()) as {
          results: ResultSummary[];
        };
        setRecentResults(resultsPayload.results);
      }
    } catch (error) {
      console.error(error);
      setLoginError(
        ...
        <section className="grid gap-4 rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-[0_30px_80px_rgba(20,38,62,0.12)] backdrop-blur md:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            <p className="inline-flex rounded-full bg-teal-100 px-4 py-1 text-sm font-semibold text-teal-900">
              Formulario de indicadores conectado a PostgreSQL
            </p>
            <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-slate-950">
              Primera versión funcional del tablero de mando con Next.js
            </h1>
            <p className="max-w-3xl text-base leading-7 text-slate-600">
              Esta pantalla ya consulta indicadores desde la base de datos y
              envía resultados reales al backend de la aplicación.
            </p>
          </div>
          ...
          <div className="grid gap-3 rounded-[1.5rem] bg-slate-950 p-5 text-slate-50">
            ...
            <p className="text-sm leading-6 text-slate-300">
              Construir una aplicación web para que diferentes usuarios consulten
              indicadores y reporten sus resultados mediante formularios.
            </p>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <StatCard label="Frontend" value="Next.js" />
              <StatCard label="Base de datos" value="PostgreSQL" />
              <StatCard label="ORM" value="Prisma" />
              <StatCard
                label="Sesión"
                value={sessionUser ? sessionUser.role : "Sin acceso"}
              />
            </div>
          </div>
        </section>
          ...
          {sessionUser ? (
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  {canCurrentUserSubmit
                    ? "Puede registrar y consultar resultados."
                    : "Puede consultar resultados, pero no registrar nuevos envios."}
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">
                  Accede para registrar resultados
                </h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Usuario inicial de prueba:
                  <br />
                  correo: <strong>admin@indicadores.local</strong>
                  <br />
                  clave: <strong>Admin123*</strong>
                  <br />
                  editor: <strong>editor@indicadores.local / Editor123*</strong>
                  <br />
                  visor: <strong>visor@indicadores.local / Viewer123*</strong>
                </p>
              </div>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Los campos cambian según el indicador seleccionado y se cargan
                  desde PostgreSQL.
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Indicador" required>
                <select
                  ...
                  required
                  disabled={!formValues.process || isLoadingTemplate}
                >
            ...
            <div className="space-y-4 rounded-[1.5rem] bg-teal-50 p-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Estos nombres vienen de la base de datos según el indicador
                  seleccionado.
                </p>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
              <p className="text-sm text-slate-500">
                El formulario guarda resultados en PostgreSQL usando rutas API y
                Prisma.
              </p>
              <button
                className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
                type="submit"
                disabled={
                  isSubmitting || isLoadingTemplate || !sessionUser || !canCurrentUserSubmit
                }
              >
          ...
          <aside className="space-y-6">
            <section className="rounded-[2rem] border border-cyan-200 bg-cyan-50 p-6 shadow-[0_16px_40px_rgba(21,94,117,0.10)]">
              ...
              <ol className="mt-4 grid gap-3 text-sm leading-6 text-slate-700">
                ...
                <li>5. La app guarda el envío en PostgreSQL.</li>
              </ol>
            </section>
              ...
              {confirmationId ? (
                <div className="mt-4 space-y-4">
                  ...
                  <p className="text-sm leading-6 text-slate-600">
                    Este número sale del registro creado por el backend.
                  </p>
                </div>
              ) : (
            ...
            <section className="rounded-[2rem] bg-slate-950 p-6 text-slate-50 shadow-[0_20px_60px_rgba(15,23,42,0.22)]">
              ...
              <div className="mt-4 grid gap-3 text-sm leading-6 text-slate-300">
                <p>1. Agregar autenticación y roles por usuario.</p>
                <p>2. Listar los registros ya creados.</p>
                <p>3. Permitir edición controlada de resultados.</p>
                <p>4. Crear panel de indicadores y reportes.</p>
              </div>
                  ...
                  {recentResults.map((result) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        {result.submittedBy || "Sin usuario"}
                      </td>
...
export { default } from "@/app/_components/formulario-indicadores";
